...
string Mining (Blokas block[], int blcount, Blockchainas blchain[], int chcount, int &n)
{
    int x;
    string newhash;
    if (blcount == 0)
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        newhash = hashas(block[blcount].DifficultyTarget + block[blcount].MerkleRootHash[0] + block[blcount].PrevBlockHash + block[blcount].TimeStamp + to_string(block[blcount].Version) + to_string(block[blcount].Nonce));
...
int main()
{
    ...
    int dydis = 10000, blcount = 0, chcount = 0, nr, j, z, n;
    string eil, newhash, q;
    users userstruct;
    ...
    while (dydis >= 100)
    {
        for (blcount = 0; blcount < 5; blcount++)
        {
        z = 0;
        srand((unsigned) time(0));
        for (int i = 0; i < 100; i++)
        ...
        while (q == "Ne")
        {
            for (int i = 0; i < 5; i++)
            {
                q = Mining(block, i, blchain, chcount, n);
            }
            n = n*2;
        }
        cout << q << endl;
        blchain[chcount].hhash = q;
        for (int i = 0 ; i < 100; i++)
        {
            int send = 0, get = 0;
